...
function App() {
  ...
  const handleDex = async () => {
    ...
    const uniqueChains = [...new Map(resp.data.data.items.map((item) => [item["chain_name"], item])).values()];
    const unqiueDex = [...new Map(resp.data.data.items.map((item) => [item["dex_name"], item])).values()];
  ...
  return (
    <div className="App" style={{ backgroundColor: `${bg}` }}>
      <Banner img={CONFIG.TEMPLATE.banner_picture !== "" ? CONFIG.TEMPLATE.banner_picture : null} head={CONFIG.TEMPLATE.title} color={vibrant} />
      ...
      <Router>
        <HashRouter basename="/">
          <Switch>
            ...
            <Route path="/" render={(props) => <Landing supported={supported} dex={defaultDex} dexVal={dexVal} enabled={geckoEnabled} covkey={key} chain={chain} {...props} light={light} vibrant={vibrant} dark={dark} />} />
          </Switch>
        </HashRouter>
      </Router>
